...
┌─────────────────────────────────────────────────────────┐
│                    Streamlit Frontend                    │
│  Home │ Dashboard │ Menu │ Stock │ AI Reports │ Sustainability │ Chat │
├─────────────────────────────────────────────────────────┤
│                    database.py                          │
│        SQLite (cafe.db) ← loaded from CSVs              │
├─────────────────────────────────────────────────────────┤
│                    ai_engine.py                         │
│        OpenRouter API → 7 models + rule-based fallback  │
└─────────────────────────────────────────────────────────┘
```

### Data Flow
```
CSV files (synthetic data) → SQLite on first run
                                    ↓
Menu + Recipes → When you log a sale, ingredients auto-deduct from stock
                                    ↓
Stock levels → Expiry alerts, low stock warnings
                                    ↓
Usage + Waste logs → Sustainability dashboard, AI analysis
                                    ↓
All data → AI Reports (raw data sent to LLM for genuine analysis)
         → AI Chat (full DB context, conversational)
```
